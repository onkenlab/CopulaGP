...
def important_copulas(model: bvcopula.Mixed_GPInferenceModel, train_x: Tensor) -> Tensor:
	...
	if len(model.likelihood.likelihoods)<2:
		...
		#print(control.shape, torch.mean(control,dim=1).shape, lowest_mixes.shape, mixes.shape)
		#torch.Size([2, 20000]) torch.Size([2]) torch.Size([2, 20000]) torch.Size([2, 1000, 20000])
		return (torch.mean(lowest_mixes,dim=1)>0.1).type(torch.bool)
		#return (torch.sum(mixes>0.1,dim=1)>0).type(torch.bool)
...
def add_copula(X: Tensor, Y: Tensor, train_x: Tensor, train_y: Tensor, device: torch.device,
	...
	for i, el in enumerate(available): #iterate over absolute indexes of available elements
		...
		try:
			...
			important = important_copulas(model,train_x)
	...
	print("Best added copula: {} {} (WAIC = {:.0f})".format(best.name,utils.strrot(best.rotation),np.max(waics)))
	logging.info("Best added copula: {} {} (WAIC = {:.0f})".format(best.name,utils.strrot(best.rotation),np.max(waics)))
	...
	weights_filename = '{}/w_{}.pth'.format(path_output,name)
	if (available_elements(reduced_likelihoods) != available_elements(best_likelihoods)):
	...
	return (reduced_likelihoods,waic)
...
def select_copula_model(X: Tensor, Y: Tensor, device: torch.device,
	...
	print("The best model is {} with WAIC = {:.0f}".format(utils.get_copula_name_string(mixtures[best_ind]),waics[best_ind]))
	...
	for mix,waic in zip(mixtures[1:],waics[1:]):
		print("{} with WAIC = {:.0f}".format(utils.get_copula_name_string(mix),waic))
